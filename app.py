...
fvg_max = st.sidebar.slider("Max FVGs to Display", min_value=1, max_value=50, value=10)
...
def find_fair_value_gaps(df):
    ...
    for i in range(1, len(df) - 1):
        ...
        if pd.isna(c1_high) or pd.isna(c3_low) or pd.isna(c1_low) or pd.isna(c3_high):
            continue

        # Use .isoformat() for clean Plotly-safe timestamps
        raw_t2   = df['Datetime'].iloc[i]
        raw_tend = df['Datetime'].iloc[-1]
        # Strip timezone if present to avoid offset suffix issues
        if hasattr(raw_t2, 'tz_localize'):
            try:
                raw_t2   = raw_t2.tz_localize(None)
                raw_tend = raw_tend.tz_localize(None)
            except TypeError:
                try:
                    raw_t2   = raw_t2.tz_convert(None)
                    raw_tend = raw_tend.tz_convert(None)
                except Exception:
                    pass

        t2    = pd.Timestamp(raw_t2).isoformat()
        t_end = pd.Timestamp(raw_tend).isoformat()
...
# ============================================================
# CHART RENDERING WITH VOLUME PROFILE + FAIR VALUE GAPS
# ============================================================
def render_chart(df, buy_x, buy_y, sell_x, sell_y, fast_period, slow_period):
    ...
    if show_fvg:
        ...
        # Show only the most recent N FVGs
        fvgs = fvgs[-fvg_max:]
        bull_drawn, bear_drawn = False, False
        for fvg in fvgs:
            if fvg["type"] == "bullish" and show_bull_fvg:
                fig.add_shape(
                    type="rect",
                ...
                # Add a single legend entry for bullish FVGs
                if not bull_drawn:
                    fig.add_trace(go.Scatter(
                        x=[None], y=[None],
                        mode='markers',
                    ...
                    bull_drawn = True
            elif fvg["type"] == "bearish" and show_bear_fvg:
                fig.add_shape(
                    type="rect",
                    x0=fvg["start_time"], x1=fvg["end_time"], xref="x",
                    y0=fvg["bottom"],     y1=fvg["top"],       yref="y",
                    fillcolor="rgba(255, 60, 60, 0.12)",
                    line=dict(color="rgba(255, 60, 60, 0.55)", width=1),
                )
                if not bear_drawn:
                    fig.add_trace(go.Scatter(
                        x=[None], y=[None],
    ...
    if show_vp:
        ...
        if vp is not None:
            ...
            fig.add_trace(go.Bar(
                x=norm_vols, y=prices,
                orientation='h', width=bar_height,
    ...
    fig.update_layout(
        ...
        xaxis2=dict(
            domain=[0.83, 1.0], showgrid=False,
            showticklabels=False, zeroline=False,
            range=[0, 1.05], fixedrange=True, autorange="reversed",
        ),
...
# ============================================================
    ...
    # ── Fixed: correct indentation, no longer inside elif block ──
    def load_all(file):
